/**
 * DIAGNOSTICS: ABSOLUTE TOP
 */
console.log("Bastion Manager | main.js script starting...");

// This sniffer is now wide-open. It will log every hook that mentions "Sheet", "Actor", "bastion", or "dnd5e".
Hooks.on("all", (hookName) => {
  const hn = hookName.toLowerCase();
    if (hn.includes("sheet") || hn.includes("actor") || hn.includes("bastion") || hn.includes("dnd5e")) {
        console.warn(`Bastion Manager | SNIFFER detected: "${hookName}"`);
    }
});
...
/**
 * INTEGRATION ENGINE: Character Sheet Injection
 */
const integrateBastionDashboard = (app, html) => {
    const actor = app.document || app.actor;
    if (!actor || actor.documentName !== "Actor" || !["character", "npc", "group"].includes(actor.type)) return;
    ...
    const attemptInjection = () => {
        const tabSelectors = [
            'section.tab.bastion[data-tab="bastion"]',
            '.tab[data-tab="bastion"]',
            '[data-tab="bastion"]'
        ];
        
        // Search for the tab content area. V13 sub-templates often need .addBack to catch the root of the fragment
        const tabContent = $html.find(tabSelectors.join(", ")).addBack(tabSelectors.join(", ")).not('.item, .anchor, button, [role="tab"]');

        if (tabContent.length > 0) {
            if (tabContent.find(".bastion-manager-suite").length > 0) return;

            console.log(`Bastion Manager | Target found for ${actor.name}. Injecting suite...`);
            ...
            const suite = `
                <div class="bastion-manager-suite" style="margin: 5px 0 15px 0; border: 1px solid #7a7971; border-radius: 5px; background: rgba(0,0,0,0.05); padding: 12px; font-family: var(--font-primary); flex: 0 0 auto; width: 100%; color: var(--color-text-dark-primary);">
                    <div style="display: flex; justify-content: space-between; align-items: center; gap: 10px;">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <i class="fa-solid fa-chess-rook" style="font-size: 2em; opacity: 0.8;"></i>
                            <div>
                                <h3 style="margin: 0; border: none; font-size: 1.2em; font-weight: bold;">Bastion Management Suite</h3>
                                <div style="font-size: 0.95em; opacity: 0.8;">Module Tracking: <b>Turn ${turnCount}</b></div>
                            </div>
                        </div>
                        <button type="button" class="launch-bastion-dashboard" style="width: auto; padding: 5px 15px; font-weight: bold; background: #333; color: white; border: 1px solid #000; border-radius: 4px; cursor: pointer;">
                            <i class="fa-solid fa-gauge-high"></i> Launch Manager
                        </button>
                    </div>
                </div>
            `;
            tabContent.prepend(suite);
            tabContent.find(".launch-bastion-dashboard").on("click", ev => {
                ev.preventDefault();
    ...
    // Initial check
    attemptInjection();
    // Small delay to catch V13 async tab rendering
    setTimeout(attemptInjection, 50);
};

/**
 * HOOK REGISTRATION
 */
Hooks.on("renderActorSheet", integrateBastionDashboard);
Hooks.on("renderCharacterSheet2", integrateBastionDashboard);
Hooks.on("renderGroupSheet2", integrateBastionDashboard);
Hooks.on("dnd5e.renderActorSheet", integrateBastionDashboard);

// Wide catch-all for V13 ApplicationV2 sheets
Hooks.on("renderApplication", (app, html) => {
    if (app.document?.documentName === "Actor" || app.constructor.name.includes("Sheet")) {
        integrateBastionDashboard(app, html);
    }
});

/**
 * D&D 5.2.x PARTIAL RENDERING HOOK
 * V13 CharacterSheet2 renders the Bastion tab as a "part".
 */
Hooks.on("renderCharacterSheet2", (app, html, data) => {
    // If dnd5e version 4+ uses specialized part hooks, this catches them
    integrateBastionDashboard(app, html);
});
...
Hooks.once("ready", async () => {
    console.log("Bastion Manager | Ready hook fired.");
...
/**
 * CONFIGURATION CLASSES (These can be outside Hooks.once("ready") but must be defined before Hooks.once("init"))
 */
